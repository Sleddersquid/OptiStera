...
template <typename T>
Max_Size_Vector<T>::Max_Size_Vector() : _max_size(50)
{
}

template <typename T>
Max_Size_Vector<T>::Max_Size_Vector(unsigned long max_size)
{
	_max_size = max_size;
}

template <typename T>
void Max_Size_Vector<T>::push_back(const T _var)
{
...
template <typename T>
std::vector<T> Max_Size_Vector<T>::getVector()
{
	return _vec;
}

template <typename T>
unsigned long Max_Size_Vector<T>::size()
{
	return _vec.size();
}

template <typename T>
T &Max_Size_Vector<T>::operator[](int i)
{
	return _vec[i];
}
...
kalmantracking::kalman::kalman(unsigned int iMode, bool bFullDebugOn)
{
	...
	// P -> Uncertainty Matrix 										10e5f -> 10e3f
...
Point kalmantracking::kalman::Predict(cv::Point2i ball)
{
	cv::Point KBall;
	...
	try
	{
		...
		if (_initialized)
		{
			...
			if (ball.x != 0 || ball.y != 0) // Do not predict
			{
				// If measured
