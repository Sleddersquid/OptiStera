 ...
 */
cv::Point calculateCenter(const cv::Mat& image) {
  ...
  return cv::Point(-1, -1); // If no center is found, return (0, 0)
}
...
int main() {
  ...
  cv::Scalar hsv_lower(0, 70, 120); // 0, 150, 50
...
//   cam.options->getExposureMode();
//   cam.options->getMeteringMode(); 
//   cam.options->getWhiteBalance();
  ...
  while (true) {
    ...
    new_center = calculateCenter(mask);

      if (new_center.x == -1 && new_center.y == -1) {
        continue;
      }
