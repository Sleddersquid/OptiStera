...
int16_t ZERO_POS[NUM_MOTORS]  = { 188}; // , 179, 189, 184 
int16_t END_POS[NUM_MOTORS]   = { 847}; // , 841, 830, 839 

// Pin group arrays; each value corresponding to the actuator (see "pin_layout.h" for specific ins)
const uint8_t DIR_PINS[NUM_MOTORS] = { DIR_PIN_6 }; // DIR_PIN_1, DIR_PIN_2, DIR_PIN_3,
const uint8_t PWM_PINS[NUM_MOTORS] = { PWM_PIN_6 }; // PWM_PIN_1, PWM_PIN_2, PWM_PIN_3,
const uint8_t POT_PINS[NUM_MOTORS] = { POT_PIN_6 }; // POT_PIN_1, POT_PIN_2, POT_PIN_3,
...
const uint8_t POS_THRESHOLD[NUM_MOTORS] = { 4 }; // 4, 4, 4,
const float P_COEFF[NUM_MOTORS]         = { 3 }; // 3, 3, 3,
const float I_COEFF[NUM_MOTORS]         = { 0.03 }; // 0.03, 0.035, 0.03,
const float D_COEFF[NUM_MOTORS]         = { 0.025 }; // 0.025, 0.025, 0.025,

// Serial configuration parameters
#define BAUD_RATE 115200  // baud rate for serial port (also needs to be set on host side)

// Serial input parameters
#define INPUT_TRIGGER 7   // at minimum, 3 numbers + 5 spaces, was 10
